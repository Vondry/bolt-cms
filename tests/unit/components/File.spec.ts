...
describe('EditorFile Component', () => {
    ...
    const filenameInput = () =>
        wrapper.find('input[name="fields[file][filename]"]');
    ...
    it('renders the filename, title, media and upload inputs', () => {
        ...
        expect(
            (wrapper.find('input[name="fields[file][media]"]').element as any)
                .value,
        ).toBe('42');
        ...
        expect((titleInput().element as any).value).toBe('A report');
        ...
        expect(wrapper.find('a.dropdown-item').attributes('href')).toBe(
            '/attributes?file=documents/report.pdf',
        );
    ...
    it('keeps the edited title in the input', async () => {
        ...
        expect((titleInput().element as any).value).toBe('Renamed report');
    });
    ...
    it('emits move events with the field name when in a filelist', async () => {
        ...
        expect(wrapper.emitted('moveFileUp')).toHaveLength(1);
        expect(wrapper.emitted('moveFileUp')[0][0].fieldName).toBe(
            'fields[file][]',
        );
        expect(wrapper.emitted('moveFileDown')).toHaveLength(1);
        expect(wrapper.emitted('moveFileDown')[0][0].fieldName).toBe(
            'fields[file][]',
        );
    });
    ...
    it('clears the inputs and emits remove when removing', async () => {
        ...
        expect((filenameInput().element as any).value).toBe('');
        expect((titleInput().element as any).value).toBe('');
    ...
    it('shows the drag overlay while dragging over the field', async () => {
        ...
        const overlayDisplay = () =>
            (wrapper.find('.editor__file--dragging').element as HTMLElement)
                .style.display;
    ...
    it('uploads a dropped file and tracks the progress', async () => {
        ...
        expect((filenameInput().element as any).value).toBe('files/new.pdf');
        expect(wrapper.find('.progress').exists()).toBe(false);
    });

    it('uploads a file picked through the file input', async () => {
        (Axios.post as any).mockResolvedValue({ data: 'files/picked.pdf' });
        wrapper = mount(FileField, { props: defaultProps });
        ...
        expect((filenameInput().element as any).value).toBe('files/picked.pdf');
    });
    ...
    it.each([
        ...
        ['an unknown response', {}, 'unknown error'],
    ])('alerts the upload failure with %s', async (_label, data, message) => {
        const alertSpy = vi.spyOn(window, 'alert').mockImplementation(() => {});
        const warnSpy = vi.spyOn(console, 'warn').mockImplementation(() => {});
        (Axios.post as any).mockRejectedValue({ response: { data } });
        ...
        expect(alertSpy).toHaveBeenCalledWith(
            message + '<br>File did not upload.',
        );
    ...
    it('browses the server files in a modal and applies the selection', async () => {
        ...
        // Known extension gets its icon, unknown extension falls back, exe is filtered out
        ...
        expect((filenameInput().element as any).value).toBe('report.pdf');
    ...
    it('navigates into a directory from the modal', async () => {
        ...
        expect((Axios.get as any).mock.calls[1][0]).toBe(directoryLink.href);
        expect(
