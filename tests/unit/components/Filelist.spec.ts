...
import FileComponent from '@/editor/Components/File.vue';
...
describe('EditorFilelist Component', () => {
    ...
    it('renders empty state hidden input when no files', async () => {
        ...
        expect(wrapper.vm.containerFiles.length).toBe(0);
    ...
    it('computes allowMore correctly', async () => {
        ...
        wrapper.vm.addFile();
        expect(wrapper.vm.containerFiles.length).toBe(3);
        expect(wrapper.vm.allowMore).toBe(false);
    });
    ...
    it('removes a file', async () => {
        ...
        expect(wrapper.vm.containerFiles.length).toBe(1);
        expect(wrapper.vm.containerFiles[0].filename).toBe('2.jpg');
    });
    ...
    it('handles undefined files', () => {
        ...
        expect(localWrapper.vm.containerFiles.length).toBe(0);
    });
